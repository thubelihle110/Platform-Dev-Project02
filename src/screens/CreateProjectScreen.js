...
export default function CreateProjectScreen({ navigation }) {
  ...
  const updateTask = (index, nextTask) => {
    setTasks((current) =>
      current.map((task, taskIndex) => (taskIndex === index ? nextTask : task))
    );
  };
  ...
  const pickCoverImage = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission needed",
        "Photo access is needed to upload a project cover image."
      );
      return;
    }
  ...
  const captureLocation = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission needed",
        "Location access is needed to tag a project site."
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim() || !category || !campus || !createdBy.trim()) {
      Alert.alert(
        "Missing details",
        "Please fill in the project name, category, campus, and creator."
      );
      return;
    }
    ...
    if (validTasks.length === 0) {
      Alert.alert(
        "Missing tasks",
        "Add at least one task title before creating the project."
      );
      return;
    }
    ...
    try {
      ...
      const detailRoute = routeNames.includes("ProjectDetailAdmin")
        ? "ProjectDetailAdmin"
        : "ProjectDetail";

      navigation.replace(detailRoute, {
        projectId,
